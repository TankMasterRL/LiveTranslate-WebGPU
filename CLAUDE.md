...
- Branches are **fast-forward merged** into `main`
  ...
  *(Transitional: until PR #1 lands, the base branch is `baseline`;
  rename it to `main` after merging, then delete this note.)*
...
- Commit messages: imperative, single-line subject, optional body
  explaining *why* (e.g. why a pipeline seam, worker-protocol shape, or
...
2. **Implement + validate.** Red/green TDD: write the failing test
   ...
   `BASE_PATH=x npm run build` when touching asset or model URLs.
...
4. **Open the PR only when asked.** PRs target
   ...
   enumerate what *must* be true for the change to ship — typically:
